<div class="container-fluid" ng-controller="adminEditController">
	<h1>Sửa đề thi <a class="small" href="/admin/test">Quay lại</a></h1>
	<!-- Nav tabs -->
...
adminApp.controller('adminEditController', ['$scope', function($scope) {
	...
	$scope.load = function() {
		$.ajax({
			url: '/api/admin/test/item/test/' + $scope.item.id,
			dataType: 'json',
	...
	$scope.save = function() {
		$.ajax({
			url: '/api/admin/test/update/test/' + $scope.item.id,
			dataType: 'json', type: 'post',
			data: {
				item: angular.copy($scope.item)
			},
			success: function(resp) {
				window.location = '/admin/test?last_id=' + $scope.item.id;
			}
		});
	};
