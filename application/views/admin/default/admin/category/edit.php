<div class="container-fluid" ng-controller="adminEditController">
	<h1>Sửa danh mục <a class="small" href="/admin/category">Quay lại</a></h1>
	<!-- Nav tabs -->
...
adminApp.controller('adminEditController', ['$scope', function($scope) {
	...
	$scope.load = function() {
		$.ajax({
			url: '/api/admin/category/item/categories/' + $scope.item.id,
			dataType: 'json',
	...
	$scope.save = function() {
		$.ajax({
			url: '/api/admin/category/update/categories/' + $scope.item.id,
			dataType: 'json', type: 'post',
			data: {
				item: angular.copy($scope.item)
			},
			success: function(resp) {
				window.location = '/admin/category?last_id=' + $scope.item.id;
			}
		});
	};
