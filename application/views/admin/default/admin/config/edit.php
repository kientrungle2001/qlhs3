<div class="container-fluid" ng-controller="adminEditController">
	<h1>Sửa <?= $controller->title?> <a class="small" href="/admin/<?= $controller->module?>">Quay lại</a></h1>
	<!-- Nav tabs -->
...
<div class="row">
	...
	<div class="row">
		<div class="col-md-4">
			<div class="row">
				<div class="col-md-8 font-weight-bold pt-1">pageSize</div>
				<div class="col-md-16"><input type="text" ng-model="mvalue.pageSize" class="form-control form-control-sm"></div>
			</div>
		</div>
		<div class="col-md-4">
			<div class="row">
				<div class="col-md-8 font-weight-bold pt-1">pageNum</div>
				<div class="col-md-16"><input type="text" ng-model="mvalue.pageNum" class="form-control form-control-sm"></div>
			</div>
		</div>
	</div>
</div>
...
adminApp.controller('adminEditController', ['$scope', function($scope) {
	...
	$scope.load = function() {
		$.ajax({
			url: '/api/admin/<?= $controller->module?>/item/<?= $controller->table ?>/' + $scope.item.id,
			dataType: 'json',
			success: function(item) {
				$scope.item = item;
				if(item.mvalue != '') {
					$scope.mvalue = JSON.parse(item.mvalue);
				}
				$scope.$apply();
			}
		});
	...
	$scope.save = function() {
		$.ajax({
			url: '/api/admin/<?= $controller->module?>/update/<?= $controller->table?>/' + $scope.item.id,
			dataType: 'json', type: 'post',
			data: {
				item: angular.copy($scope.item)
			},
			success: function(resp) {
				window.location = '/admin/<?= $controller->module?>?last_id=' + $scope.item.id;
			}
		});
	};
